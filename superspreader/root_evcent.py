...
if __name__ == '__main__':
    ...
    evcent_df = pd.read_csv('E:\\ViralPred\\weibo_network\\evcent.csv',index_col=0,header=None,sep=',')

    rtuids = rtuid_df[1].astype('int')
    #print rtuids[:5]
    rtuid_evcent = evcent_df.loc[rtuids][1]

    #print rtuid_kshell[:5]
    fs_df[3] = pd.Series(rtuid_evcent.values,index=fs_df.index)
    print len(fs_df[1])
    print fs_df.head()
    fs_df.to_csv('E:\\ViralPred\\weibo_network\\evcent_design_matrix_50.csv',header=None,index=False)
