...
if __name__ == '__main__':
    ...
    for i in xrange(0,10):
        ...
        for tr_ind,ts_ind in kf:
            if clf == 'SVM':
                lr = SVC(cache_size=20000,kernel='rbf',C=10000,probability=False)
            if clf == 'LSVM':
                lr = LinearSVC()
            if clf == 'LR':
        ...
        of = open(p+'clf_res\\asonam_TH_'+str(int(TH))+'_'+clf+'C1.csv','ab')
        wrt = csv.writer(of)
        wrt.writerow([c1_precision,c1_recall,c1_f1,c2_precision,
                      c2_recall,c2_f1,weighted_prec,weighted_rec,weighted_f1])

        of.close()
    time_of = open(p+'run_time\\asonam_clf_'+str(clf)+'_train_tes1tC1.csv','ab')
