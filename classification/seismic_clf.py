...
def design_matrix_handle(p_and_fname,y_max):

    number_of_samples = 99257

    seismic_f = open(p_and_fname,'rb')
    X = [[] for i in xrange(0,number_of_samples)]

    line_cnt = 0

    for line in seismic_f:
        raw_row = line[:-1].split(',')
        for ele in raw_row:
            ele = float(ele)
            if ele < 50 or np.isnan(ele):
                ele = 50
            elif np.isinf(ele) or ele>y_max:
                ele = y_max

            X[line_cnt%number_of_samples].append(ele)
            line_cnt += 1
...
if __name__ == '__main__':
    ...
    if DS == 'Twitter':
        ...
        #f_name = 'asonam_features_lambda1800.csv'
        fs_col_no = 3
    ...
    if DS == 'Weibo':
        ...
        db_name = 'Aminer'
        q = 'SELECT * FROM fs_df'
        #f_name = 'asonam_features.csv'
        f_name = 'seismic_pred_weibo_300.txt'
        fs_col_no = 2

    TH = TH0

    seismic_df = pd.read_csv(p+f_name,index_col=0,header=None)
    ...
    print len(fs_df),len(seismic_df)
    ...
    X = design_matrix_handle(p+f_name,np.max(fs_list))
    ...
    for i in xrange(0,10):
        ...
        of = open(p+'clf_res\\seismic_clf_TH_'+str(int(TH))+'_'+clf+'.csv','ab')
        wrt = csv.writer(of)
    ...
    time_of = open(p+'run_time\\seismic_clf_'+str(clf)+'_train_test.csv','ab')
