...
if __name__ == '__main__':
    ...
    seismic_df = pd.read_csv('E:\\ViralPred\\weng_network\\rpp_GD3600.csv',index_col=0,header=None)

    print seismic_df.head()
    print len(fs_df),len(seismic_df)

    fs_df = fs_df.loc[seismic_df.index.values]
    fs_list = fs_df[3].values
    fs_list = [x for x in fs_list if int(x) >= 50]
    max_fs = np.max(fs_list)
    ...
    names = seismic_df.index.values

    X = []

    for row_raw in X_raw:
        row = []
        for x in row_raw:
            if math.isnan(x) or x < 50.0:
                x = 50.0
            elif x > max_fs or math.isinf(x):
                x = max_fs
            row.append(x)
        X.append(row)
    X = np.asarray(X)
    ...
    j = 0
    of = open('E:\\ViralPred\\weng_network\\clf_res\\rpp_clf_TH_'+str(int(TH))+'_'+clf+'.csv','wb')
    wrt = csv.writer(of)
